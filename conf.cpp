...
int main()
{
	try {
		...
		for (std::vector<Config::ConfigServer>::iterator it = servers.begin();
			...
			std::list<Config::ConfigServer::ConfigLocation> reglocs = it->getRegexLocations();
			std::cout << "\tserver has " << reglocs.size() << " reg locations"
			          << std::endl;

			for (std::list<Config::ConfigServer::ConfigLocation>::iterator itt = reglocs.begin();
			     itt != reglocs.end(); itt++) {
				std::cout << "\t\tname: " << itt->getName() << std::endl;
				std::cout << "\t\troot: " << itt->getRoot() << std::endl;
